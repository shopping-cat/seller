...
const review = () => {
    ...
    const { data, loading } = useItemReviews({ variables: { id: Number(query.id) } })
    ...
    return (
        <div>
            <Space direction='horizontal' style={{ marginBottom: 16 }} >
                <h1>총 리뷰 수 : {data.itemReviews.length}</h1>
                <h1>평균 평점 : {data.item.rate}</h1>
            </Space>
            <Table
                columns={[
                    {
